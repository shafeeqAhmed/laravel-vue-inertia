                        ...
                        <div class="space-y-6 sm:space-y-5">
                            ...
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                ...
                                <div class="mt-1 sm:mt-0 sm:col-span-2">
                                        ...
                                        <div class="mt-1 relative">
                                            <ListboxButton class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                <span class="flex items-center">
                                                    <img v-if="form.worker_id != null" :src="workers[workers.findIndex((worker) => worker.id===form.worker_id)].avatar" alt="" class="flex-shrink-0 h-6 w-6 rounded-full" />
                                                    <span v-if="form.worker_id != null" class="ml-3 block truncate">{{ workers[workers.findIndex((worker) => worker.id===form.worker_id)].forenames }} {{ workers[workers.findIndex((worker) => worker.id===form.worker_id)].surname }}</span>
                                            ...
                                            <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                                                <ListboxOptions class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                                    <ListboxOption as="template" v-for="worker in workers" :key="worker.id" :value="worker.id" v-slot="{ active, selected }">
                                    ...
                                    <div v-if="form.errors.worker_id" v-text="form.errors.worker_id" class="text-red-500 text-xs mt-1"></div>
                                </div>
                            </div>
                            ...
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                ...
                                <div class="mt-1 sm:mt-0 sm:col-span-2">
                                    <RadioGroup v-model="form.attendee_type_id">
                                        ...
                                        <div class="space-y-4">
                                            <RadioGroupOption as="template" v-for="attendee_type in attendee_types" :key="attendee_type.id" :value="attendee_type" v-slot="{ checked, active }">
                            ...
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                ...
                                <div class="mt-1 sm:mt-0 sm:col-span-2">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">Notes about the attendee, not included in published output</p>
